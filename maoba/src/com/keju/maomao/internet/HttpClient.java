...
import com.keju.maomao.Configuration;
...
public class HttpClient implements java.io.Serializable {
	...
	// private static final int NOT_MODIFIED = 304;// Not Modified(未修改):
	// 没有返回响应时无新数据返回
	// private static final int BAD_REQUEST = 400;// Bad Request(错误请求):
	// 请求不合法，服务器不理解请求的语法。
	// private static final int NOT_AUTHORIZED = 401;// Not Authorized(未授权):
	// 请求要求身份验证。
	...
	// private static final int NOT_ACCEPTABLE = 406;// Not Acceptable(不接受):
	// 无法使用请求的内容特性响应请求的资源
	private static final int INTERNAL_SERVER_ERROR = 500;// Internal Server
															// Error(服务器内部错误):
															// 服务器遇到错误，无法完成请求。
	// private static final int BAD_GATEWAY = 502;// Bad Gateway(错误网关):
	// 服务器作为网关或代理，从上游服务器收到无效响应。
	// private static final int SERVICE_UNAVAILABLE = 503;// Service
	// Unavailable(服务不可用): 服务器目前无法使用（由于超载或停机维护）。 通常，这只是暂时状态。请重试。
	...
	protected Response httpRequest(String url, PostParameter[] postParams) throws SystemException {
		...
		if (postParams != null) {
			...
			// PostParameter("PCode",Constants.PRODUCT_ID);
		}
