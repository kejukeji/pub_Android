 ...
 * 
 * @author zhuoyong
 * @data 创建时间：2013-12-16 下午5:58:25
 */
public class MyGreetingActivity extends BaseActivity implements OnClickListener {
	private ImageButton ibLeft;
	private Button btnRight;
	private TextView tvTitle;

	private ListView lvgreetingList;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.my_greeting_list);
	...
	private void findView() {
		...
		tvTitle.setText("眉目传情");

		lvgreetingList = (ListView) this.findViewById(R.id.lvgreetingList);
		tvTitle = (TextView) this.findViewById(R.id.tvTitle);
	...
	private void fillData() {
		...
		lvgreetingList.addFooterView(vFooter); // 注意
												// 此句话必须要在setAdapter前面才可以显示上拉查看更多
		lvgreetingList.setAdapter(adapter);
		lvgreetingList.setOnScrollListener(LoadListener);
		lvgreetingList.setDivider(null);
		lvgreetingList.setFooterDividersEnabled(false);

		if (NetUtil.checkNet(MyGreetingActivity.this)) {
					...
					try {
						...
						if (status == Constants.REQUEST_SUCCESS) {
							...
							if (result.has("greeting")) {
								JSONArray arr = result.getJSONArray("greeting");
								if (arr != null) {
	...
	public class Adapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				convertView = getLayoutInflater().inflate(R.layout.my_greeting_item, null);
			...
			Drawable cacheDrawble = AsyncImageLoader.getInstance().loadDrawable(url, new ImageCallback() {
				...
				public void imageLoaded(Drawable imageDrawable, String imageUrl) {
					ImageView image = (ImageView) lvgreetingList.findViewWithTag(imageUrl);
					if (image != null) {
