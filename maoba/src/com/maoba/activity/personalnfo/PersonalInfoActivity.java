...
import com.maoba.util.DateUtil;
		...
		if (resultCode == RESULT_OK) {
			...
			case Constants.PHOTO_PICKED_WITH_DATA:// 相册
				...
				try {
					...
					String birthday1 = "";
					if (NetUtil.checkNet(PersonalInfoActivity.this)) {
						new personInfoAddTask(nickname1, birthday1, sex1, signature1, address1, newPassword1).execute();
					}else{
						showShortToast(R.string.NoSignalException);
					}
				} catch (Exception e) {
			...
			if (loginType == 0) {
				...
				try {
					return new BusinessHelper().thirdAddUserInfor(userId, loginType, openId, nickName, birthday,
							gender, signature, address, avatarFile);
				} catch (SystemException e) {
